import type { VercelRequest, VercelResponse } from '@vercel/node'
import { createClient } from '@supabase/supabase-js'

const SUPABASE_URL = process.env.SUPABASE_URL as string
const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY as string

const admin = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, { auth: { autoRefreshToken: false, persistSession: false } })

export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method !== 'POST') {
    res.status(405).json({ success: false, error: 'Method not allowed' })
    return
  }

  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    res.status(500).json({ success: false, error: 'Missing Supabase environment configuration' })
    return
  }

  const authHeader = String(req.headers.authorization || '')
  if (!authHeader) {
    res.status(401).json({ success: false, error: 'Missing authorization' })
    return
  }

  const { userId, companyId } = req.body || {}
  if (!userId || !companyId) {
    res.status(400).json({ success: false, error: 'Missing userId or companyId' })
    return
  }

  try {
    const userRes = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
      headers: {
        Authorization: authHeader,
        apikey: SUPABASE_SERVICE_ROLE_KEY
      }
    })

    if (!userRes.ok) {
      res.status(401).json({ success: false, error: 'Invalid token' })
      return
    }

    const caller = await userRes.json()
    const callerId = caller?.id as string | undefined
    if (!callerId) {
      res.status(401).json({ success: false, error: 'Invalid token' })
      return
    }

    const { data: roleRow, error: roleError } = await admin
      .from('company_collaborators')
      .select('role')
      .eq('company_id', String(companyId))
      .eq('user_id', callerId)
      .maybeSingle()

    if (roleError) {
      res.status(400).json({ success: false, error: roleError.message })
      return
    }

    const role = (roleRow as any)?.role as string | undefined
    const allowedRoles = new Set(['owner', 'admin', 'master'])
    if (!role || !allowedRoles.has(role)) {
      res.status(403).json({ success: false, error: 'Access denied' })
      return
    }

    const { error: deleteError } = await admin.auth.admin.deleteUser(String(userId))
    if (deleteError) {
      const msg = String(deleteError.message || '')
      if (!msg.toLowerCase().includes('not found')) {
        res.status(400).json({ success: false, error: msg })
        return
      }
    }

    res.status(200).json({ success: true })
  } catch (e: any) {
    res.status(500).json({ success: false, error: e?.message || 'Server error' })
  }
}

